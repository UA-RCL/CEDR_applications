...
//for(int iii=0; iii<10; iii++){   
    task_configs["task2-CEDR_FFT_flt-1"] = NULL;
    cedr_task_config_t *&confp1 = task_configs["task2-CEDR_FFT_flt-1"];
    auto task2 = taskflow.emplace([&c, &X1, len, argc, argv, &confp1](){
    // lol scope
    {
    ...
    }).name("task2-CEDR_FFT_flt-1");

    task_configs["task3-CEDR_FFT_flt-1"] = NULL;
    cedr_task_config_t *&confp2 = task_configs["task3-CEDR_FFT_flt-1"];
    auto task3 = taskflow.emplace([&d, &X2, len, argc, argv, &confp2](){  
    // lol scope
    {
      cedr_cmplx_flt_type *fft_inp = (cedr_cmplx_flt_type*) malloc(len * sizeof(cedr_cmplx_flt_type));
      cedr_cmplx_flt_type *fft_out = (cedr_cmplx_flt_type*) malloc(len * sizeof(cedr_cmplx_flt_type));
    ...
    }).name("task3-CEDR_FFT_flt-1");

    auto task4 = taskflow.emplace([&X1, &X2, len, &corr_freq]() {
      for (size_t i = 0; i < 2 * len; i += 2) {
          corr_freq[i] = (X1[i] * X2[i]) + (X1[i + 1] * X2[i + 1]);
          corr_freq[i + 1] = (X1[i + 1] * X2[i]) - (X1[i] * X2[i + 1]);
      }
    }).name("task4-non_kernel");

    task_configs["task5-CEDR_FFT_flt-1"] = NULL;
    cedr_task_config_t *&confp3 = task_configs["task5-CEDR_FFT_flt-1"];
    auto task5 = taskflow.emplace([&corr_freq, &corr, len, argc, argv, &confp3](){
    // lol scope
    {
      cedr_cmplx_flt_type *fft_inp = (cedr_cmplx_flt_type*) malloc(len * sizeof(cedr_cmplx_flt_type));
      cedr_cmplx_flt_type *fft_out = (cedr_cmplx_flt_type*) malloc(len * sizeof(cedr_cmplx_flt_type));
    ...
    }).name("task5-CEDR_FFT_flt-1");

    task4.succeed(task2,task3);
    task4.precede(task5);
    #ifdef DAG_PARSE
