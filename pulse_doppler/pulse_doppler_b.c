...
void xcorr(double *x, double *y, size_t n_samp, double *corr) {
  ...
  // lol scope
  {
    ...
    cedr_cmplx_flt_type *fft_out_x1 = (cedr_cmplx_flt_type*) malloc(len * sizeof(cedr_cmplx_flt_type));
    cedr_cmplx_flt_type *fft_inp_x2 = (cedr_cmplx_flt_type*) malloc(len * sizeof(cedr_cmplx_flt_type));
    cedr_cmplx_flt_type *fft_out_x2 = (cedr_cmplx_flt_type*) malloc(len * sizeof(cedr_cmplx_flt_type));
    bool is_fwd = true;
    ...
    cedr_FFT_flt(fft_inp, fft_out, len, true );
    cedr_FFT_flt_nb(&fft_inp_x1, &fft_out_x1, &len, &is_fwd, &barrier);
    cedr_FFT_flt_nb(&fft_inp_x2, &fft_out_x2, &len, &is_fwd, &barrier);
    ...
    pthread_mutex_unlock(barrier.mutex);*/

		{
				...
				{
				    CEDR_FFT_flt(fft_inp_x1, fft_out_x1, len, is_fwd);
				}
				

				{
				    CEDR_FFT_flt(fft_inp_x2, fft_out_x2, len, is_fwd);
				}
		}
  ...
  //   cedr_FFT_flt(fft_inp, fft_out, len, true /* is_forward_transform? */);
  ...
  // lol scope
  {
    ...
    // cedr_FFT_flt(fft_inp, fft_out, len, false /* is_forward_transform? */);
    /*pthread_cond_t cond = PTHREAD_COND_INITIALIZER;
    pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
    uint32_t completion_ctr = 0;
    ...
    cedr_FFT_flt_nb(&fft_inp, &fft_out, &len, &is_fwd, &barrier);
    ...
    pthread_mutex_unlock(barrier.mutex);*/
    CEDR_FFT_flt(fft_inp, fft_out, len, is_fwd);
    for (size_t i = 0; i < len; i++) {
...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < num_ffts; i++) {
    ...
    CEDR_FFT_flt(fft_inp[i], fft_out[i], m, is_fwd);
