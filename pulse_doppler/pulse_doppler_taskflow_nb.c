...
void general_func(cedr_re_flt_type *x, cedr_re_flt_type *y, size_t n_samp, cedr_re_flt_type **q, size_t m, size_t n_samples) {
  ...
  int first_p1 = 0;
  int last_p1 = m;
  task_configs["phase1-CEDR_FFT_flt-" + std::to_string(m)] = NULL;
  cedr_task_config_t *&confp1 = task_configs["phase1-CEDR_FFT_flt-" + std::to_string(m)];
  auto phase1 = taskflow.for_each_index(std::ref(first_p1), std::ref(last_p1), 1, [c, &X1, len, argc, argv, &confp1](int k){
...
//  for(int k=first_p1; k<last_p1; k++){
    ...
    //CEDR_FFT_flt(fft_inp_x1, fft_out_x1, len, is_fwd, .argc=argc, .argv=argv, .config=confp1, .task_id=k);
  }).name("phase1-CEDR_FFT_flt-" + std::to_string(m));
//  }

  int first_p2 = 0;
  int last_p2 = m;
  task_configs["phase2-CEDR_FFT_flt-" + std::to_string(m)] = NULL;
  cedr_task_config_t *&confp2 = task_configs["phase2-CEDR_FFT_flt-" + std::to_string(m)];
  auto phase2 = taskflow.for_each_index(std::ref(first_p2), std::ref(last_p2), 1, [d, &X2, len, argc, argv, &confp2](int k){
//  #pragma omp parallel for schedule(SCHEDULER) num_threads(THREADS)
//  for(int k=first_p2; k<last_p2; k++){
    ...
    //CEDR_FFT_flt(fft_inp_x2, fft_out_x2, len, is_fwd, .argc=argc, .argv=argv, .config=confp2, .task_id=k);
  }).name("phase2-CEDR_FFT_flt-" + std::to_string(m));
//  }
...
//  for(int k=first_ct; k<last_ct; k++){
  ...
  }).name("zips-non_kernel");
//  }

  int first_p3 = 0;
  int last_p3 = m;
  task_configs["phase3-CEDR_FFT_flt-" + std::to_string(m)] = NULL;
  cedr_task_config_t *&confp3 = task_configs["phase3-CEDR_FFT_flt-" + std::to_string(m)];
  auto phase3 = taskflow.for_each_index(std::ref(first_p3), std::ref(last_p3), 1, [corr_freq, &corr, len, argc, argv, &confp3](int k){
//  #pragma omp parallel for schedule(SCHEDULER) num_threads(THREADS)
//  for(int k=first_p3; k<last_p3; k++){
    ...
    //CEDR_FFT_flt(fft_inp, fft_out, len, is_fwd, .argc=argc, .argv=argv, .config=confp3, .task_id=k);
  }).name("phase3-CEDR_FFT_flt-" + std::to_string(m));
//  }
...
//  for(int k=first_a2D; k<last_a2D; k++){
  ...
  }).name("array2D-non_kernel");
//  }

  int first_p4 = 0;
  int last_p4 = num_ffts;
  task_configs["phase4-CEDR_FFT_flt-" + std::to_string(m)] = NULL;
  cedr_task_config_t *&confp4 = task_configs["phase4-CEDR_FFT_flt-" + std::to_string(m)];
  auto phase4 = taskflow.for_each_index(std::ref(first_p4), std::ref(last_p4), 1, [mf, &q, m, num_ffts, argc, argv, &confp4](int k){
//  #pragma omp parallel for schedule(SCHEDULER) num_threads(THREADS)
//  for(int k=first_p4; k<last_p4; k++){
    ...
    //CEDR_FFT_flt(fft_inp, fft_out, m, is_fwd, .argc=argc, .argv=argv, .config=confp4, .task_id=k);
    free(l);
  }).name("phase4-CEDR_FFT_flt-" + std::to_string(num_ffts));
//  }

  auto dopplerShift = taskflow.emplace([num_ffts, m, q, n_samples](){
    ...
    free(r);
  }).name("dopplerShift-non_kernel");
//}


  corr_task.succeed(phase1, phase2);
  corr_task.precede(phase3);
  phase3.precede(array2D);
  array2D.precede(phase4);
  phase4.precede(dopplerShift);
  #if defined(DAG_PARSE)
