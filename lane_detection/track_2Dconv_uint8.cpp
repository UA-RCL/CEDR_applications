...
void gaussian_blur(uint8_t *img, int kernel_size, int height, int width, uint8_t *return_array){
    double r, g, p, s, e;
    float sum = 0;
    ...
    CEDR_CONV_2D_int(img, height, width, gaussian_kernel, kernel_size, return_array);
}
...
void sobel_g_operator(uint8_t *img, float *filter, int kernel_size, int height, int width, uint8_t *return_array){
    CEDR_CONV_2D_int(img, height, width, filter, kernel_size, return_array);
}
...
void sobel(uint8_t *gx, uint8_t *gy, int height, int width, int lower_threshold, int upper_threshold, uint8_t *return_array){
...
// width --> image width
// Output: return_array --> edge detected frame
void roi(int height, int width, uint8_t *return_array){
...
std::vector<Line> hough_transform(uint8_t *img, int height, int width, float rho, float theta, int threshold, int line_length, int line_gap){
...
void draw_line(int x1, int y1, int x2, int y2, int width, int dimension, int color, uint8_t *return_array){
    int dx = x2 - x1;
    int dy = y2 - y1;
    int yi = 1;
    if(dy < 0){
...
void draw_circle(int *center, int r, int color, int height, int width, int depth, uint8_t *return_array){
    int r_sq = r * r;
    int x_diff, y_diff;
    for(int y = 0; y < height; y++){
        for(int x = 0; x < width; x++){
...
void draw_lane(std::vector<Line> lines, int height, int width, uint8_t * return_array, int *avg_left_return, int *avg_right_return){
...
void save_img(uint8_t* img, int height, int width, std::string name){
...
int main(){
    ...
    uint8_t* /*rgb_imag*/img = stbi_load(imagepath, &width, &height, &depth, 3);
    if (img/*rgb_imag*/ == NULL){
    ...
    uint8_t *temp_arr = (uint8_t *) malloc (sizeof (uint8_t) * (size * 3));
    //uint8_t *img = (uint8_t *) malloc (sizeof (uint8_t) * (size * 3));
    uint8_t *grayscale_img = (uint8_t *) malloc (sizeof (uint8_t) * size);
    uint8_t *blurred_img = (uint8_t *) malloc (sizeof (uint8_t) * size);
    uint8_t *blurred_img_before_canny = (uint8_t *) malloc (sizeof (uint8_t) * size);
    uint8_t *gx_canny = (uint8_t *) malloc (sizeof (uint8_t) * size);
    uint8_t *gy_canny = (uint8_t *) malloc (sizeof (uint8_t) * size);
    uint8_t *sobel_img = (uint8_t *) malloc (sizeof (uint8_t) * size);
    uint8_t *lanes = (uint8_t *) malloc (sizeof (uint8_t) * size);
    ...
    /*for (int i = 0; i<(height * width * 3); i++){
		...
		#if defined(PRINT_IMAGES)
	   stbi_write_png("redline.png", width, height, 3, img/*rgb_imag*/, width*3);	
      ...
      save_img(blurred_img_before_canny, height, width, "blurred_img_before_canny");
		#endif
 	  stbi_write_png("output_2dconv_uint8.png", width, height, 3, img, width*3);
