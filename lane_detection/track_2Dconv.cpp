...
void gaussian_blur(cedr_re_int_type *img, int kernel_size, int height, int width, cedr_re_int_type *return_array){
    double r, g, p, s, e;
    float sum = 0;
    float gaussian_kernel[kernel_size * kernel_size];
    /*std::cout << "First 20 elements of input array:" << std::endl;
    for (int i = 0; i < 20 && i < height * width; ++i) { // Check bounds to avoid out-of-bounds access
        std::cout << img[i] << " ";
        if ((i + 1) % 5 == 0) // Print 5 elements per line for better readability
            std::cout << std::endl;
    }*/
    // Computation of gaussian filter
    //TODO: if we want to improve performance, we can implement a static definition of the two kernel sizes (5, 11) and use an if statement instead of conducting calculations every
    for(int x = 0; x < kernel_size; x++){
        for(int y = 0; y < kernel_size; y++){
            ...
            e = std::exp(p);
            g = e / s;
            gaussian_kernel[x*kernel_size + y] = g;
            sum += g;
    ...
    CEDR_CONV_2D_int(img, height, width, gaussian_kernel, kernel_size, return_array,.argc=ARGC,.argv=ARGV);
   // Print first 20 elements of the return_array
    /*std::cout << "First 20 elements of return array:" << std::endl;
    for (int i = 0; i < 20 && i < height * width; ++i) {
...
void sobel_g_operator(cedr_re_int_type *img, float *filter, int kernel_size, int height, int width, cedr_re_int_type *return_array){
    CEDR_CONV_2D_int(img, height, width, filter, kernel_size, return_array,.argc=ARGC,.argv=ARGV);
}
