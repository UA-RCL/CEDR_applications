...
int main(int argc, char** argv) {
  ...
  for (i = 0; i < Nslow; i++) {
    KERN_ENTER(make_label("FFT[1D][%d][complex][float64][forward]", Nfast));
    CEDR_FFT_flt((s0+i*Nfast), (fft_out_0+i*Nfast), fast, true,.argc=argc, .argv=argv);
    KERN_EXIT(make_label("FFT[1D][%d][complex][float64][forward]", Nfast));
...
*/
    ...
    CEDR_FFT_flt(&(fft_inp_1[i*Nfast]), &(fft_out_1[i*Nfast]), fast, false,.argc=argc, .argv=argv);
  ...
  for (i = 0; i < Nfast; i++) {
    ...
    CEDR_FFT_flt(&(fft_inp_3[i*Nslow]), &(S1[i*Nslow]), slow, true,.argc=argc, .argv=argv);
...
*/
    ...
    CEDR_FFT_flt((fft_inp_2+i*Nslow), (fft_out_2+i*Nslow), slow, false,.argc=argc, .argv=argv);
