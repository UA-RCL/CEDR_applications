...
int main(int argc,char** argv) {
   ...
   FILE *cfp, *fp;
   //FILE *txdata_file; // = fopen("/localhome/jmack2545/rcl/cedr-SoC/TraceAtlas/Applications/wifi/build/txdata_1.txt", "w");
  ...
  //  }
   ...
   //txdata_file = fopen("/localhome/jmack2545/rcl/cedr-SoC/TraceAtlas/Applications/wifi/build/txdata_1.txt", "w");
  ...
  clock_gettime(CLOCK_MONOTONIC_RAW, &start);
      ...
      for(i=0; i<SYM_NUM; i++) {
          ...
          fft_id = 1;
              ...
              */
              ...
              //printf("[WIFITX-nk] Enqueuing cedr_IFFT in CEDR\n");

              //cedr_cmplx_flt_type *ifft_input, *ifft_output;
              //ifft_input = (cedr_cmplx_flt_type*) malloc(sizeof(cedr_cmplx_flt_type) * FFT_N);
              //ifft_output = (cedr_cmplx_flt_type*) malloc(sizeof(cedr_cmplx_flt_type) * FFT_N);
      ...
      for(i=0; i<SYM_NUM; i++) {
        ...
        //cedr_FFT_flt_nb(&ifft_input[i], &ifft_output[i], &size, &forwardTrans, &barrier);
      }
     ...
     /* pthread_mutex_lock(barrier.mutex);
      ...
      for(i=0; i<SYM_NUM; i++) { 
        //printf("[WIFITX-nk] Back from cedr_IFFT in CEDR\n");
        free(ifft_input[i]);
...
void readConfig() {
    ...
    //cfp = fopen("/localhome/jmack2545/rcl/cedr-SoC/TraceAtlas/Applications/wifi/build/tx.cfg", "r");
